	...
	int nCmdShow)
{
	...
	while (TRUE)
	{
		...
		else
		{
			...
			dxrr->Update(deltaTime);  // actualizar lógica y colisiones
			dxrr->Render();           // renderizar frame
		}
...
LRESULT CALLBACK WindowProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_MOUSEMOVE: {
		...
		bool puedeAvanzar = !dxrr->Colision();

		//if (keyboardData[DIK_W] & 0x80) dxrr->isPointOutsideSphere(dxrr->camara->getPos(), dxrr->coordenadas) ? (!dxrr->Colision() ? dxrr->vel = 5.0f : dxrr->vel = 0.0f) : dxrr->vel = 0.0f;
		if (keyboardData[DIK_W] & 0x80) {
			if (dxrr->isPointOutsideSphere(dxrr->camara->getPos(), dxrr->coordenadas) && !dxrr->Colision()) {
				dxrr->vel = 5.0f;
			}
			else {
				dxrr->vel = 0.0f;
		...
		if (keyboardData[DIK_S] & 0x80) dxrr->vel = -5.0f;

		if (keyboardData[DIK_A] & 0x80) dxrr->strafe = 5.0f;

		if (keyboardData[DIK_D] & 0x80) dxrr->strafe = -5.0f;

		if (keyboardData[DIK_B] & 0x80) dxrr->breakpoint = true;
		...
		// Mouse move
		dxrr->izqder = (mouseData.lX / 2000.0f);
		dxrr->arriaba = -(mouseData.lY / 2000.0f);
		...
		if (gamePad->IsConnected())
		{
			...
			if (grados > 0.19 || grados < -0.19) dxrr->izqder = grados / 50;

			grados = (float)gamePad->GetState().Gamepad.sThumbRY / 32767.0;

			if (grados > 0.19 || grados < -0.19)dxrr->arriaba = grados / 50;
