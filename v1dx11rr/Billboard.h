...
public:
	...
	void DrawHUD(const D3DXMATRIX& view, const D3DXMATRIX& projection,
		...
		int frame)
	{
		// 1. Configurar los vértices con las coordenadas UV correctas
		VertexComponent hudVertices[4];

		hudVertices[0].pos = D3DXVECTOR3(-1.0f * scale, -1.0f * scale, z);
		hudVertices[0].UV = D3DXVECTOR2(uv1[frame].u, uv1[frame].v);

		hudVertices[1].pos = D3DXVECTOR3(-1.0f * scale, 1.0f * scale, z);
		hudVertices[1].UV = D3DXVECTOR2(uv2[frame].u, uv2[frame].v);

		hudVertices[2].pos = D3DXVECTOR3(1.0f * scale, 1.0f * scale, z);
		hudVertices[2].UV = D3DXVECTOR2(uv3[frame].u, uv3[frame].v);

		hudVertices[3].pos = D3DXVECTOR3(1.0f * scale, -1.0f * scale, z);
		hudVertices[3].UV = D3DXVECTOR2(uv4[frame].u, uv4[frame].v);

		// 2. Actualizar el vertex buffer
		D3D11_MAPPED_SUBRESOURCE mappedResource;
		d3dContext->Map(vertexBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
		memcpy(mappedResource.pData, hudVertices, sizeof(VertexComponent) * 4);
		...
		// 5. Configurar texturas y samplers
		d3dContext->PSSetShaderResources(0, 1, &colorMap);
		d3dContext->PSSetSamplers(0, 1, &colorMapSampler);

		// 6. Calcular y configurar matrices
		D3DXMATRIX world;
		...
		d3dContext->UpdateSubresource(worldCB, 0, 0, &wvp, 0, 0);
		d3dContext->VSSetConstantBuffers(0, 1, &worldCB);
